...
class Model:
    ...
    #         trainer = pl.Trainer(**self.config.trainer_config, accelerator='gpu', devices=1)
    #     else:
    #         trainer = pl.Trainer(
    ...
    def gen_trainer(self):
        self.config.trainer_config.pop('gpus', None)          # drop deprecated PL-1.x arg if present
        if self.config.use_gpu:
            trainer = pl.Trainer(**self.config.trainer_config, accelerator='gpu', devices=1)
        else:
            trainer = pl.Trainer(**self.config.trainer_config, log_every_n_steps=1)
        return trainer    
